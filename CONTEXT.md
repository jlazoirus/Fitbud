...
- REQ-83 agrego motor puro de reemplazos en `js/nutrition-domain.js`: `rankReplacementCandidates(meal, candidates, mealTarget, catalog)` ranquea candidatos por score solver + proximidad kcal; `solveReplacement(changedSlotId, deltaKcal, dayMeals, dayLog)` determina si |deltaKcal|>50 y reune comidas futuras no hechas; `rebalanceFutureMeals(...)` queda como capa de compatibilidad. En `index.html`: `openChangeMeal` llama rankReplacementCandidates; `_buildChangeMealOpts` muestra los 4 deltas de macro y aviso de rebalanceo; `revertMeal` deshace rebalanceos seguros. Tests: `scripts/validate-nutrition-replacements.mjs` (10 tests). REQ-142 conecto el rebalanceo de `applyChangeMeal` a `finalizeDayWithGate()` (helper `mealEntryForFinalize()`, compartido con `lockedMealsForDay()`): futuras candidatas entran como `proposal` y `globalClosePass()` cierra el dia por hill-climbing en vez de `-deltaKcal/n`.
...
- REQ-128 agrego `DIET_CONTRACT` en `js/nutrition-domain.js`: kcal ±3% o ±50 kcal, proteína ±5 g bilateral, carbohidratos ±8 g y grasa ±8 g, con kcal autoritativa de `ingredients.kcal`. Sigue en calibración (`runtimeActive:false`) hasta REQ-139. `scripts/validate-diet-contract.mjs` mide la matriz 2/4/6 comidas × patrón dietario × proteína normal/alta × disgustos × 7 días.
...
- REQ-137 agrego `globalClosePass()` a `finalizeNutritionDay()` en `js/nutrition-domain.js`: hill-climbing sobre los macros del dia completo (no por comida) usando todas las lineas escalables de comidas no bloqueadas, respetando `lineLimits()`/`clampStep()`; si el dia sigue fuera de `DIET_CONTRACT`, `attemptContractComplement()` agrega un snack/batido compatible que reduzca el residual sin romper restricciones. Canario sube de 39/378 (10.3%) a 122/378 (32.3%); el resto son `catalog_gap` (`failureBreakdown` en `validate-diet-contract.mjs`), no bugs del solver.
...
- REQ-138 conecto `generateOneDay()`, `deterministicDayPayload()`, `generateDeterministicWeek()`, `regenerateDayInWeekDraft()` y `regenerateGenMeal()` al helper `finalizeDayWithGate()` → `finalizeNutritionDay()`; `prepareFirstWeekNutrition()` hereda la conexion. `lockedMealsForDay()` protege comidas `done=true`. El "ok" visible usa `finalizedDayIsComplete()` (slots cubiertos), no `DIET_CONTRACT`. El prompt ya no exige verificar totales; `validateGeneratedDay()` paso sus 2 checks de tolerancia de `issues` a `warns`. Validador: `validate-nutrition-finalize-wiring.mjs`. Reemplazos (`applyChangeMeal`) quedan fuera, extraidos como REQ-142.
...
- REQ-143 (crecer catalogo para subir el canario) quedo pausado 2026-07-08 tras dos lotes medidos con el diff de REQ-144 (2026-07-05 y 2026-07-08) que confirman un techo cercano a 32-33% bajo la seleccion local actual (`scoreMacros()` pondera kcal/proteina mucho mas que carbohidratos); ver REQ-147 en REQUIREMENTS.md para la decision de producto pendiente. REQ-143 se saco de `agent-loop.json` hasta esa decision.
