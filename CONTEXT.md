...
- Catalogo Supabase: ingredientes, platos, recetas, dietas y vista `dish_macros`. `supabase/seed.sql` carga el catalogo base expandido: 200 ingredientes, 180 platos y 654 lineas de receta tras REQ-141.
...
- REQ-83 agrego el motor puro de reemplazos en `js/nutrition-domain.js`: `rankReplacementCandidates()` ranquea por score solver + proximidad kcal, `solveReplacement()` decide si hace falta rebalanceo (|deltaKcal|>50) y `rebalanceFutureMeals()` queda como capa de compatibilidad. En `index.html`: `openChangeMeal`, `_buildChangeMealOpts` (4 deltas de macro + aviso) y `revertMeal`. REQ-142 conecto el rebalanceo de `applyChangeMeal` a `finalizeDayWithGate()` (helper `mealEntryForFinalize()`), asi que `globalClosePass()` cierra el dia en vez de repartir `-deltaKcal/n`. Tests: `scripts/validate-nutrition-replacements.mjs`.
...
- REQ-128 agrego `DIET_CONTRACT` en `js/nutrition-domain.js`: kcal ±3% o ±50 kcal, proteina ±5 g, carbohidratos ±8 g y grasa ±8 g, con kcal autoritativa de `ingredients.kcal`. Activo como aviso suave no bloqueante desde REQ-139 (`runtimeActive:true`). `scripts/validate-diet-contract.mjs` mide la matriz 2/4/6 comidas × dieta × proteina normal/alta × disgustos × 7 dias.
- REQ-129 agrego `finalizeNutritionDay(ctx)` en `js/nutrition-domain.js`: puerta pura dormida que normaliza propuestas por catálogo, rellena slots con `planDeterministicNutritionDay()`, respeta `lockedMeals` y reporta `contract` sin activar runtime. Test: `scripts/validate-finalize-nutrition-day.mjs`.
- REQ-130 hizo coherentes las preferencias duras (`dislikedIngredients` bloquea por defecto en dominio/cliente/proxy, ya no es preferencia blanda) y el patrón omnívoro exige (warning/reintento, sin 422 duro) al menos una comida con proteína animal salvo que el usuario excluya carnes/pescado. `COACH_PROMPT_VERSION=7`.
...
- REQ-135 agrego lote 1 de catalogo en `supabase/seed.sql`: 60 ingredientes nuevos con fuentes resumidas en `docs/catalog-lote-1-sources.md` y 50 platos nuevos (desayunos, snacks/batidos, almuerzos y cenas ligeras). `slot_without_candidates` desaparece del canario. Cobertura por slot y baselines de cada lote: `docs/catalog-lote-*-sources.md`.
- REQ-136 agrego `dishes.cuisine_tags` (`criolla/mediterranea/mexicana/asiatica`) en schema + semantica idempotente; `preferenceScoreAdjustment()` prioriza suavemente `prefs.preferredCuisines` via `cuisineTagsForDish()`/`preferredCuisineTags()`, el editor admin preserva tags y `COACH_PROMPT_VERSION=9` invalida pools por hash de catalogo con cocina. Tests: `scripts/validate-cuisine-preferences.mjs` y cobertura por cocina en `validate-nutrition-catalog.mjs`.
- REQ-140 agrego lote 2B en `supabase/seed.sql`: 41 ingredientes y 45 platos nuevos (fuentes en `docs/catalog-lote-2b-sources.md`), con cobertura de cocina en desayuno/snack/plato principal para las 4 cocinas (`criolla` 2/2/12, `mediterranea` 3/4/12, `mexicana` 2/2/9, `asiatica` 2/3/9) y mas opciones needs_kitchen=false/eat_out_ok/budget bajo/alta proteina sin proteina en polvo. `inferCuisineTags` (migracion + espejo de test) ahora tambien reconoce `criolla`/`criollo` en el nombre, no solo los platos de almuerzo del menu semanal. Canario `validate-diet-contract.mjs` sigue en calibracion hasta REQ-137.
- REQ-141 agrego lote 2C en `supabase/seed.sql`: 38 ingredientes y 35 platos nuevos (200 ingredientes, 180 platos, 654 lineas de receta en total; fuentes en `docs/catalog-lote-2c-sources.md`). `validate-nutrition-catalog.mjs` exige 180/200 y cobertura de escenarios; las estimaciones sin lacteos/sin gluten son heuristicas, no certificacion de alergenos. Canario: 39/378 (10.3%) con `finalizeNutritionDay`.
- REQ-137 agrego `globalClosePass()` a `finalizeNutritionDay()`: hill-climbing sobre los macros del dia completo (no por comida) con todas las lineas escalables de comidas no bloqueadas, respetando `lineLimits()`/`clampStep()`; si el dia sigue fuera de contrato, `attemptContractComplement()` suma un snack/batido compatible. Subio el canario de 39/378 a 122/378; su lectura de "el resto es `catalog_gap`" resulto incorrecta (ver REQ-166).
- REQ-144 agrego `scripts/diff-diet-contract.mjs`: compara dos `seed.sql` contra la matriz de 378 dias y reporta delta total/por dimension antes de aceptar lotes de catalogo. Guia: `docs/diet-contract-catalog-diff.md`.
- REQ-138 conecto `generateOneDay()`, `deterministicDayPayload()`, `generateDeterministicWeek()`, `regenerateDayInWeekDraft()` y `regenerateGenMeal()` a `finalizeDayWithGate()` → `finalizeNutritionDay()`; `prepareFirstWeekNutrition()` lo hereda y `lockedMealsForDay()` protege comidas `done=true`. El "ok" visible usa `finalizedDayIsComplete()` (slots cubiertos), no `DIET_CONTRACT`. Validador: `validate-nutrition-finalize-wiring.mjs`. Reemplazos: REQ-142.
- REQ-139 activo `runtimeActive=true`: `dietContractNoticeText()` agrega aviso suave no bloqueante en revision y al aplicar. Validador: `validate-diet-contract-runtime-notice.mjs`.
- REQ-166 cambio el objetivo de `globalClosePass()`: puntua con `scoreDayAgainstContract()` (cada metrica en multiplos de su tolerancia de `DIET_CONTRACT`, al cuadrado) y ya no con `scoreMacros()`, que pesa kcal x1.4 y proteina hasta x1.45 contra x0.55 de carbohidratos/grasa y por eso compraba kcal/proteina pagando con carbohidratos. No cambia que plato gana cada slot: `scoreMacros()` sigue siendo el score por comida. Canario 122/378 -> 289/378 (76.5%), 34 dimensiones mejoran, 0 regresionan. El objetivo debe seguir siendo continuo: uno escalonado deja al hill-climbing sin gradiente (205/378). Validador: `scripts/validate-global-close-objective.mjs`.
- REQ-143 sigue pausado, pero el "techo de catalogo 32-33%" que lo motivaba no existia (era el objetivo de cierre). Sus criterios requieren re-basarse contra 289/378. El hueco real son las 7 dimensiones en 0/7, todas de 2 comidas mas vegano/`sin_tofu`. REQ-147 cerrado por decision de Jonathan (2026-08-30).
