...
- REQ-138 conecto `generateOneDay()`, `deterministicDayPayload()`, `generateDeterministicWeek()`, `regenerateDayInWeekDraft()` y `regenerateGenMeal()` al helper `finalizeDayWithGate()` → `finalizeNutritionDay()`; `prepareFirstWeekNutrition()` hereda la conexion. `lockedMealsForDay()` protege comidas `done=true`. El "ok" visible usa `finalizedDayIsComplete()` (slots cubiertos), no `DIET_CONTRACT` (dormido). El prompt ya no exige verificar totales; `validateGeneratedDay()` paso sus 2 checks de tolerancia de `issues` a `warns`. Validador: `validate-nutrition-finalize-wiring.mjs`. Reemplazos (`applyChangeMeal`) quedan fuera, extraidos como REQ-142.
- REQ-139 redefinido: aviso suave no bloqueante, ya no depende del catalogo, listo para implementar (primero en `agent-loop.json` desde 2026-07-08). REQ-143 (crecer catalogo para subir el canario) quedo pausado 2026-07-08 tras dos lotes medidos con el diff de REQ-144 (2026-07-05 y 2026-07-08) que confirman un techo cercano a 32-33% bajo la seleccion local actual (`scoreMacros()` pondera kcal/proteina mucho mas que carbohidratos); ver REQ-147 en REQUIREMENTS.md para la decision de producto pendiente. REQ-143 se saco de `agent-loop.json` hasta esa decision.
