...
- REQ-141 agrego lote 2C en `supabase/seed.sql`: 38 ingredientes y 35 platos nuevos (200 ingredientes, 180 platos, 654 lineas de receta en total; fuentes en `docs/catalog-lote-2c-sources.md`), cerrando la meta final de profundidad del REQ-136 original. `scripts/validate-nutrition-catalog.mjs` ahora exige explicitamente `REQ141_MIN_DISHES=180`/`REQ141_MIN_INGREDIENTS=200` y cobertura minima de escenarios `needs_kitchen=false` (>=40), `eat_out_ok` (>=25), y estimaciones de "sin lacteos"/"sin gluten" por categoria de ingrediente y una lista explicita de ingredientes con gluten conocido (`GLUTEN_INGREDIENTS`); son heuristicas de catalogo, no certificacion de alergenos ni reemplazo del filtrado real de restricciones. Canario `validate-diet-contract.mjs` sube de 27/378 (7.1%) a 39/378 (10.3%) con `engine:"finalizeNutritionDay"`; el cierre del 98% sigue siendo responsabilidad de REQ-137.
...
- REQ-139 redefinido: aviso suave no bloqueante. REQ-143 bloqueado 2026-07-05, ver REQUIREMENTS.md. Siguiente: REQ-144.
