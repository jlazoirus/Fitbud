...
- REQ-128 agrego `DIET_CONTRACT` en `js/nutrition-domain.js`: kcal ±3% o ±50 kcal, proteína ±5 g bilateral, carbohidratos ±8 g y grasa ±8 g, con kcal autoritativa de `ingredients.kcal`. `validateDietContractTotals()` y `dietContractTolerance()` son solo calibración (`runtimeActive:false`); no cambian `validateDayTotals`, cliente, servidor ni snapshots hasta REQ-129. `scripts/validate-diet-contract.mjs` reconstruye el catálogo desde `supabase/seed.sql` + semántica REQ-79 y mide la matriz 2/4/6 comidas × patrón dietario × proteína normal/alta × disgustos × 7 días; baseline 2026-07-04: 0/378 días dentro de contrato, así que REQ-129 debe cerrar macros globalmente antes de activar runtime.
