...
- **Privacidad y seguridad:** `user_consents` y `safety_screenings` guardan aceptación y aptitud por versión. Edad mínima 18 años. Sin consentimiento vigente no se crea/adapta el plan ni se llama al coach; una señal de alerta reemplaza la rutina por una pausa segura.
...
- **Onboarding:** `renderOnboarding()` guía datos corporales → macros → entrenamiento → logística de comidas → preferencias, consentimientos y evaluación de seguridad. `migrateProfilePrefs()` normaliza el esquema v2; `hasCompleteOnboarding()` no bloquea perfiles heredados y `profileReviewDue()` solicita revisión cada 28 días.
...
- **Entrenamiento:** cada perfil elige disciplina, fuerza, 3-6 días exactos, lugar por día, minutos, equipo, experiencia, prioridad y limitaciones. `workoutSchedule()` asigna fuerza/deporte; `effectiveWorkout(ds)` conserva overrides y devuelve `safety_hold` si la evaluación tiene una señal de alerta.
...
- **RLS (con `auth.sql` + `plan_cycles.sql` + `privacy.sql`):** escritura anónima eliminada. Datos, planes, consentimientos y evaluaciones solo permiten acceso propio; catálogo solo a admins activos. Las APIs admin/privacidad usan service role exclusivamente en servidor.
