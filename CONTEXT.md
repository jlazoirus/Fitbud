...
- REQ-141 agrego lote 2C en `supabase/seed.sql`: 38 ingredientes y 35 platos nuevos (200 ingredientes, 180 platos, 654 lineas de receta en total; fuentes en `docs/catalog-lote-2c-sources.md`), cerrando la meta final de profundidad del REQ-136 original. `scripts/validate-nutrition-catalog.mjs` ahora exige explicitamente `REQ141_MIN_DISHES=180`/`REQ141_MIN_INGREDIENTS=200` y cobertura minima de escenarios `needs_kitchen=false` (>=40), `eat_out_ok` (>=25), y estimaciones de "sin lacteos"/"sin gluten" por categoria de ingrediente y una lista explicita de ingredientes con gluten conocido (`GLUTEN_INGREDIENTS`); estas dos ultimas son estimaciones de composicion del catalogo para planificar REQ-137, no una certificacion de alergenos ni reemplazan el filtrado real de restricciones del usuario. Canario `validate-diet-contract.mjs` sube de 27/378 (7.1%) a 39/378 (10.3%) con `engine:"finalizeNutritionDay"`; el cierre del 98% sigue siendo responsabilidad de REQ-137.
